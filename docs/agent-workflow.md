...
- `relo project show [--json]`, `relo -v`, `relo --version`: inspect project metadata or print the build version.
- `relo upgrade [--check]` or `relo upgrade --version VERSION`: check or install an official GitHub Release independently of project state. Archive checksums are verified, and detected package-manager installations must be upgraded with their manager.
...
- `relo graph [--format tree|json] [--tasks-only | --all-milestones]`: inspect deterministic task graph shape and status summary. Tree output includes planned/ready checkpoint anchors by default; `--all-milestones` adds marked history and `--tasks-only` suppresses checkpoints. These tree-view flags never change task-DAG legality; graph JSON remains task-only and rejects both tree-view flags.
